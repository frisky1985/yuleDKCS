...
// ═════════════════════════════════════════════════════════════
// KeyRepository (SQL + Redis) Tests — using sqlmock + miniredis
// ═════════════════════════════════════════════════════════════

func newKeyRepoSQL(t *testing.T) (*KeyRepository, sqlmock.Sqlmock, *miniredis.Miniredis) {
	t.Helper()
	...
	repo := NewKeyRepository(db, redisClient)
	return repo, mock, mr
}
...
func keyColumns() []string {
	return []string{"id", "vehicle_id", "user_id", "key_type", "status",
		"permissions", "secret", "parent_key_id", "created_at",
		"activated_at", "expires_at", "revoked_at", "revoke_reason", "metadata"}
}

// ─────────────────────────────────────────────────────────────
// NewKeyRepository Test
// ─────────────────────────────────────────────────────────────

func TestNewKeyRepository_SQL(t *testing.T) {
	...
	repo := NewKeyRepository(sqlxDB, redisClient)
	if repo == nil {
		t.Fatal("NewKeyRepository should return non-nil")
	}
	mr.Close()
}
...
func TestKeyRepoSQL_Create_Success(t *testing.T) {
	...
	mock.ExpectQuery(
		`INSERT INTO digital_keys \(id,vehicle_id,user_id,key_type,status,permissions,secret,parent_key_id,created_at,expires_at,metadata\) VALUES \(\?,\?,\?,\?,\?,\?,\?,\?,\?,\?,\?\) RETURNING id`,
	).WithArgs(
	...
	if cached.ID != key.ID {
		t.Errorf("cached ID: want %q, got %q", key.ID, cached.ID)
	}
...
func TestKeyRepoSQL_Create_DBError(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	key := testKeySQL()
...
func TestKeyRepoSQL_GetByID_SQL(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	key := testKeySQL()
	...
	mock.ExpectQuery(`SELECT \* FROM digital_keys WHERE id = \? LIMIT 1`).
		WithArgs(key.ID).
		WillReturnRows(sqlmock.NewRows(keyColumns()).AddRow(
			key.ID, key.VehicleID, key.UserID, key.KeyType, key.Status,
			permJSON, key.Secret, key.ParentKeyID, key.CreatedAt,
			nil, key.ExpiresAt, nil, "", metaJSON,
		))
...
func TestKeyRepoSQL_GetByID_CacheHit(t *testing.T) {
	...
	got, err := repo.GetByID(ctx, key.ID)
	if err != nil {
		t.Fatalf("GetByID from cache failed: %v", err)
	}
	if got.ID != key.ID {
		t.Errorf("ID: want %q, got %q", key.ID, got.ID)
	}
...
func TestKeyRepoSQL_GetByID_DBError(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	...
	_, err := repo.GetByID(ctx, "err-key")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestKeyRepoSQL_Update_Success(t *testing.T) {
	...
	mock.ExpectExec(
		`UPDATE digital_keys SET status = \?, permissions = \?, activated_at = \?, revoked_at = \?, revoke_reason = \?, metadata = \? WHERE id = \?`,
	).WithArgs(
		key.Status, permJSON, key.ActivatedAt, key.RevokedAt, key.RevokeReason, metaJSON, key.ID,
	...
	_, err = repo.getCachedKey(ctx, key.ID)
	if err == nil {
...
func TestKeyRepoSQL_Update_NoRows(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	key := testKeySQL()
...
func TestKeyRepoSQL_Update_DBError(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	key := testKeySQL()
...
func TestKeyRepoSQL_ListByUser_Success(t *testing.T) {
	...
	mock.ExpectQuery(
		`SELECT \* FROM digital_keys WHERE user_id = \? ORDER BY created_at DESC LIMIT 10 OFFSET 0`,
	).WithArgs(key.UserID).
		WillReturnRows(sqlmock.NewRows(keyColumns()).AddRow(
			key.ID, key.VehicleID, key.UserID, key.KeyType, key.Status,
			permJSON, key.Secret, key.ParentKeyID, key.CreatedAt,
	...
	result, err := repo.ListByUser(ctx, key.UserID, 10, 0)
	if err != nil {
		t.Fatalf("ListByUser failed: %v", err)
	}
	if len(result) != 1 {
		t.Fatalf("want 1 key, got %d", len(result))
	}
	if result[0].ID != key.ID {
		t.Errorf("ID: want %q, got %q", key.ID, result[0].ID)
	}
...
func TestKeyRepoSQL_ListByUser_Empty(t *testing.T) {
	...
	result, err := repo.ListByUser(ctx, "no-keys-user", 10, 0)
	if err != nil {
		t.Fatalf("ListByUser failed: %v", err)
	}
	if len(result) != 0 {
...
func TestKeyRepoSQL_ListByVehicle_Success(t *testing.T) {
	...
	permJSON, _ := json.Marshal([]string{"unlock"})
	metaJSON, _ := json.Marshal(map[string]interface{}{})
	now := time.Now().Truncate(time.Millisecond)

	mock.ExpectQuery(
		`SELECT \* FROM digital_keys WHERE vehicle_id = \? ORDER BY created_at DESC LIMIT 5 OFFSET 0`,
	).WithArgs("v1").
		WillReturnRows(sqlmock.NewRows(keyColumns()).AddRow(
			"k1", "v1", "u1", "primary", "active",
			permJSON, "secret", nil, now,
			nil, now.Add(24*time.Hour), nil, "", metaJSON,
		).AddRow(
			"k2", "v1", "u2", "friend", "active",
			permJSON, "secret2", strPtr("k1"), now,
			nil, now.Add(24*time.Hour), nil, "", metaJSON,
		))

	result, err := repo.ListByVehicle(ctx, "v1", 5, 0)
	if err != nil {
		t.Fatalf("ListByVehicle failed: %v", err)
	}
	if len(result) != 2 {
		t.Fatalf("want 2 keys, got %d", len(result))
	}
...
func TestKeyRepoSQL_ListActiveByVehicle_Success(t *testing.T) {
	...
	mock.ExpectQuery(
		`SELECT \* FROM digital_keys WHERE vehicle_id = \? AND status = \? AND expires_at > \? ORDER BY created_at DESC`,
	).WithArgs("v1", "active", sqlmock.AnyArg()).
		WillReturnRows(sqlmock.NewRows(keyColumns()).AddRow(
			"k1", "v1", "u1", "primary", "active",
			permJSON, "secret", nil, now,
			nil, now.Add(24*time.Hour), nil, "", metaJSON,
	...
	result, err := repo.ListActiveByVehicle(ctx, "v1")
	if err != nil {
		t.Fatalf("ListActiveByVehicle failed: %v", err)
	}
	if len(result) != 1 {
	...
	if result[0].ID != "k1" {
		t.Errorf("ID: want k1, got %s", result[0].ID)
	}
...
func TestKeyRepoSQL_CacheInvalidation(t *testing.T) {
	repo, _, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	key := testKeySQL()
	...
	cached, err := repo.getCachedKey(ctx, key.ID)
	if err != nil {
		t.Fatalf("cache should have key: %v", err)
	}
	if cached.ID != key.ID {
...
func TestKeyRepoSQL_CacheKeyID(t *testing.T) {
	repo, _, mr := newKeyRepoSQL(t)
	defer mr.Close()

	key := repo.cacheKeyID("key-123")
	expected := "key:key-123"
	if key != expected {
...
// ─────────────────────────────────────────────────────────────
// listKeys scan error test
// ─────────────────────────────────────────────────────────────

func TestKeyRepoSQL_listKeys_ScanError(t *testing.T) {
	repo, mock, mr := newKeyRepoSQL(t)
	defer mr.Close()
	ctx := context.Background()
	...
	mock.ExpectQuery(
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "vehicle_id"}).AddRow("k1", "v1"))

	_, err := repo.ListByUser(ctx, "u1", 10, 0)
	if err == nil {
...
func TestKey_HasPermission_SQL(t *testing.T) {
	key := &Key{Permissions: []string{"unlock", "start"}}
	if !key.HasPermission("unlock") {
		t.Error("should have unlock")
	}
