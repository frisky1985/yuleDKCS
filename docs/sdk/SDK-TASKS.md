...
| # | 任务 | 说明 | 依赖 | 并行性 |
|:-:|:-----|:-----|:----:|:----:|
| 4.1 | SDK 单元测试（Mock Hub、Mock TCU） | iOS XCTest + Android JUnit | ✅ **完成 (2026-08-01)**: 覆盖审计 `docs/sdk/TEST-COVERAGE-AUDIT.md`; 补齐远程控车请求形状 (iOS+Android) + Push 回调/KeyManager 离线推断 (双端); iOS parse/typecheck, Android 33/33 交叉验证; 发现 iOS 模块预存编译错误 4 处 (见 TASK_STATUS) | 无 | ✅ 可与 2b 并行 |
| 4.2 | SDK × Hub 集成测试（真实 gRPC 调用） | 复用现有 E2E 测试框架；REST 层已实测打通（2026-07-31） | ✅ **完成 (2026-08-01)**: `scripts/sdk-hub-contract-e2e.sh` 扩展至 11 段 21 断言 (新增 remoteLock/Unlock/CreateShare/AcceptShare/CancelShare/UnbindKey), 全量跑通 exit 0; 关键契约事实: sendCommand 端点 snake_case 需特判, 分享状态机/UnbindKey 记录保留为 hub stub 缺口 | 无 | ✅ 可与 2b 并行 |
...
| 4.5 | ICCOA/ICCE S2S 分享全链路 E2E | 通过 Hub mock S2S | ✅ **完成 (2026-08-01)**: e2e_12 (ICCOA 5/5) + e2e_13 (ICCE 6/6) 全 PASS; 修复 e2e_14 预存编译错误 + e2e_11 隐性回归 (relay Delete 终态语义); 全量 integration **87 passed / 0 failed**; 纯 mock 无外部依赖 | 无 | ✅ 可与 2b 并行 |
